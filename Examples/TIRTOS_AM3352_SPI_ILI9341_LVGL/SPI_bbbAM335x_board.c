...
void SpiInit(void)
{
    ...
    spiCfg.chnCfg[spiCfg.chNum].trMode = MCSPI_TX_ONLY_MODE;
    ...
    /* Diagnostic: read CH1CONF */
    {
        uint32_t ch1conf = HWREG(SOC_SPI_0_REGS + MCSPI_CH1CONF);
        UART_printf("CH1CONF=0x%08X CLKD=%u CLKG=%u DPE0=%u DPE1=%u\n",
            ch1conf,
            (ch1conf >> 2) & 0xF,
            (ch1conf >> 29) & 1,
            (ch1conf >> 16) & 1,
