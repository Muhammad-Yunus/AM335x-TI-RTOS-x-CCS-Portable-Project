/**
 * @file    lv_port_disp.c
 * @brief   LVGL display port for ILI9341 on AM3352 (SPI1 + DMA)
 */
...
/* Use the same SPI handle and RX buffer as main.c */
extern SPI_Handle gSpiLcdHandle;
extern uint8_t lcdRxBuf[];
...
/* Forward declare SPI write helpers from main.c */
                             ...
                             uint8_t *color_p)
{
    ...
    LcdDcLow();
    Spi1TxByte(ILI9341_CMD_COLADDR);
    LcdDcHigh();
    ...
    LcdDcLow();
    Spi1TxByte(ILI9341_CMD_PAGEADDR);
    LcdDcHigh();
    ...
    LcdDcLow();
    Spi1TxByte(ILI9341_CMD_GRAM);
    LcdDcHigh();

    uint32_t send_size =
        (uint32_t)(x2 - x1 + 1) * (uint32_t)(y2 - y1 + 1) * LCD_FB_BPP;

    CacheP_Inv((void *)color_p, (int32_t)send_size);
    CacheP_wb((void *)color_p, (int32_t)send_size);

    Spi1TxBuffer(color_p, send_size);
